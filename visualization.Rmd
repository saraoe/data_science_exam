...
Loading the data
```{r}
df <- read_csv("data_science_exam/data/model_df.csv")   # making the first changepoint 1 instead of 0
```
...
for (cp in unique(df$change_point)){
    ...
    ggsave(paste("data_science_exam/fig/corr_mat_cp_", cp, ".png", sep = ""), plot, width=6, height=6)
}
...
ggsave("data_science_exam/fig/corr_mat.png", better_patch, width=7, height=7)
```
...
```{r}
# values = pal_rickandmorty("schwifty")(10)
# values
...
raw_emotions
ggsave("data_science_exam/fig/raw_emotions.png", raw_emotions, width=8, height=5)

```


```{r}
p_load(ggthemes, permute, scales)
values = list(tableau_color_pal("Classic 10 Medium")(8))
vals = c(values[[1]][4], 
        values[[1]][7], 
        values[[1]][2], 
        values[[1]][3], 
...
values[[1]][1]
print(values)
print(rev(sort(values[[1]])))

show_col(tableau_color_pal("Classic 10 Medium")(8))
unique(emos_melt$variable)
```nt(values)
print(rev(sort(values[[1]])))
