...
for cp in df["change_point"].unique():
    emos = df[df["change_point"]==cp][labels] # ["date", "change_point"]+
    corr_mat = emos.corr()
    sns.heatmap(corr_mat, annot=True, cmap="coolwarm", vmin=-1, vmax=1)
    plt.title(f'Correlation matrix in change point period {cp+1}', fontdict={"size":14})
