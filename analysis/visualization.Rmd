...
patch
# ggsave("fig/rn_slopes_2022.png", patch, width=8, height=8)
```
...
emos = df %>% subset(date > '2019-12-19' & date < '2020-01-01') %>% select(labels)
corr_mat = melt(round(cor(emos),3))
...
ggsave("fig/corr_mat_xmas2019.png", plot, width=6, height=6)
```

```{r}
df <- read_csv("/home/saram/hope-twitter/emoDynamics/idmdl/hope_model_df.csv")   # making the first changepoint 1 instead of 0
```
...
emos = df %>% subset(date > '2021-12-20' & date < '2022-01-01') %>% select(labels)
corr_mat = melt(round(cor(emos),3))
...
ggsave("hope-twitter/data_science_exam/fig/corr_mat_xmas2021.png", plot, width=6, height=6)
```
