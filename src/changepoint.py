...
def write_model_df(
    ...
    model_df = pd.DataFrame(
        columns=["change_point", "resonance", "novelty", "transience"] + labels
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in_filepath", type=str, required=True, help="Filepath for the input file."
    )
    parser.add_argument(
        "--out_filepath", type=str, required=True, help="Filepath for the output file."
    )
    parser.add_argument(
        "--penalty",
        type=str,
    ...
    df = pd.read_csv(args.in_filepath)
    out_path = args.out_filepath
    pen = args.penalty
    labels = [
        "happiness",
        "trust",
